...
const Work = () => {
  ...
  const [usingCMSData, setUsingCMSData] = useState(false);
  ...
  const refreshData = () => {
    ...
    setTimeout(() => {
      try {
        ...
        console.log("Manual refresh - CMS Projects found:", cmsProjects.length);
        ...
        if (cmsProjects.length > 0) {
          ...
          setUsingCMSData(true);
          alert(
            `Refreshed! Loaded ${convertedProjects.length} projects from CMS.`,
          );
        } else {
          setPortfolioProjects(defaultPortfolioProjects);
          setUsingCMSData(false);
          alert(
            `No CMS data found. Using ${defaultPortfolioProjects.length} default projects.`,
          );
        }
      } catch (error) {
        console.error("Error during manual refresh:", error);
        alert("Error refreshing data. Using default projects.");
        setPortfolioProjects(defaultPortfolioProjects);
        setUsingCMSData(false);
      } finally {
  ...
  useEffect(() => {
    const loadProjects = () => {
      ...
      try {
        // Try to get CMS projects
        const cmsProjects = getPublishedProjects();
        console.log("CMS Projects found:", cmsProjects.length);
        ...
        if (cmsProjects.length > 0) {
          ...
          console.log("Using CMS data:", convertedProjects.length, "projects");
          setPortfolioProjects(convertedProjects);
          setUsingCMSData(true);
        } else {
          ...
          console.log(
            ...
            defaultPortfolioProjects.length,
            "projects",
          );
          setPortfolioProjects(defaultPortfolioProjects);
          setUsingCMSData(false);
    ...
    // Also listen for localStorage changes (when CMS updates)
    ...
    const interval = setInterval(() => {
      const currentData = localStorage.getItem("portfolio_projects");
      if (currentData) {
        loadProjects();
      }
    }, 3000);
        ...
        <div className="max-w-6xl mx-auto text-center relative z-10">
          ...
          >
            ...
            <div className="flex items-center gap-1 bg-black/50 backdrop-blur-sm rounded-full px-3 py-1 text-xs">
              ...
              <span className="text-gray-300">
                {isLoadingCMS
                  ? "Loading..."
                  : usingCMSData
                    ? `CMS (${portfolioProjects.length})`
                    : `Default (${portfolioProjects.length})`}
              </span>
            </div>
            ...
            <motion.button
              ...
              className="flex items-center gap-1 bg-black/50 backdrop-blur-sm rounded-full px-3 py-1 text-xs text-gray-300 hover:text-beige transition-colors disabled:opacity-50"
              title="Refresh portfolio data"
            >
              <RefreshCw
                className={`w-3 h-3 ${isLoadingCMS ? "animate-spin" : ""}`}
              />
              <span>Refresh</span>
            </motion.button>
